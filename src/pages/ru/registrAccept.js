...
btnAccept.addEventListener("click", function() {
    ...
    if (isNotKyr(inputValue[0].value)){
        ...
        infoMess += "Логин не должен содержать символы кирилицы и так же не должен быть пустым.";
    };
    // if (RegExp('[a-zA-Z0-9]+@gmail/.com').exec(inputValue[1].value)){

    // }else {
    //     infoMess += "/nАдреса введена не вiрно. Приймаеться тiльки формат @gmail.com .";
    // };
    if (isNotKyr(inputValue[1].value)){
        infoPlayer.push(inputValue[1].value)
    }else{
        infoMess += "\nПароль не должен содержать символы кирилицы и так же не должен быть пустым.";
    };
    ...
    if (infoMess != "") {
        ...
        // }else{
        document.location.href = '../game/ThreeLaborsOfHermes.html?json=' + infoGame;
        };
    // };
});